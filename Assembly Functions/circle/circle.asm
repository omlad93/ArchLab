		...
		beq $imm, $zero, $s2, exit     # if r == 0 go to exit
		add $t1, $zero, $imm, 21
		...
		beq $imm, $zero, $zero, fill1  # fill the pixels in the current layer
top:     
		add $t0, $t0, $imm, 1          # layer += 1
		bgt $imm, $t0, $s2, second     # if (layer > r) go to second
		add $s0, $zero, $imm, 175      # x = 175
		add $s1, $zero, $imm, 143      # y = 143
		sub $s0, $s0, $s2, 0           # x = x - r
...
buttom:     
		add $t0, $t0, $imm, 1          # layer += 1
		bgt $imm, $t0, $s2, exit       # if (layer > r) go to exit
		add $s0, $zero, $imm, 175      # x = 175
		add $s1, $zero, $imm, 143      # y = 143 
		sub $s0, $s0, $s2, 0           # x = x - r
		add $s0, $s0, $t0, 0           # x = x + layer
...
fill2:
		...
		add $t3, $t3, $imm, 1          # i += 1
		add $s0, $s0, $imm, 1          # x += 1
		beq $imm, $t3, $t2, buttom     # if (i == n) go to the next layer
