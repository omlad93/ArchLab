...
typedef struct sp_registers_s {
	...
	// counter of dma transaction
	int dma_counter;

	// 1 bit indicator for dma status : 1 is busy
	int dma_status;
	
	// 32 bit source address for dma
	int dma_src;
...
void dma_call(sp_t *sp){
	...
	switch(spro->dma_state){
		...
		case DMA_STATE_HOLD:
			sprn->dma_state = (mem_is_busy) ? DMA_STATE_HOLD: DMA_STATE_READ;
			break;
		...
		case DMA_STATE_COPY:
			mem_extract = llsim_mem_extract_dataout(sp->sram, 31, 0);
			llsim_mem_set_datain(sp->sram, mem_extract, 31, 0);
			llsim_mem_write(sp->sram, spro->dma_dst);
			dma_cpy_step(spro,sprn);
			break;
	}
