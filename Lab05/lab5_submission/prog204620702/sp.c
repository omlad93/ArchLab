...
#define dma_hazard_reg(reg_idx, spro) (((spro->exec0_active) && (spro->exec0_dst == reg_idx)) || ((spro->exec1_active) && (spro->exec1_dst == reg_idx)) )
#define dma_bypass_dec1(dec1,exec0,exec1,r) ((dec1==exec0) ? (r[exec0]) : (dec1 == exec1) ? (r[exec1]) : (r[dec1]))
// #define dma_bypass_dec1(dec1,exec0,exec1,r) ((dec1==exec0) ? (1) : (dec1 == exec1) ? (2) : (0))
#define not(x) (!(x))
...
void print_trace_file(sp_t* sp)
{
	...
	case JNE:
		...
		break;
	case CMB: // DMA
		fprintf(inst_trace_fp, ">>>> EXEC: CMB  %d words from address %04x to address %04x <<<\n", sp->spro->dma_size , sp->spro->dma_src, sp->spro->dma_dst);
		break;
	case POL: // DMA
...
int stall_dec1_is_needed(sp_registers_t* spro, sp_t *sp){
	int condition1 = (src_is_real(spro->dec1_src0) && (data_hazard_mem(spro, spro->dec1_src0) || dma_hazard_reg(spro->dec1_src0,spro)));
	int condition2 = (src_is_real(spro->dec1_src1) && (data_hazard_mem(spro, spro->dec1_src1) || dma_hazard_reg(spro->dec1_src1,spro)));
...
/*
	MAKE SURE PREPERATION RAISES WORK IN DEC1 BUT EXECUTE IN EXEC0
*/
...
void dma_prepare_next_copy(sp_registers_t *spro, sp_registers_t *sprn, int block_dma){
	...
	// sp_printf("DMA copyied %i / %i (%i->%i)\n\n", spro->dma_counter + 1,spro->dma_size, spro->dma_src, spro->dma_dst);
